...
private:
    ...
    //开始解析一行中开始的空格和tab
    // src: 源串
    // 返回值: 有空格数和有内容处的char*指针组成的pair类型
    ...
    inline pair<int, char *> JudgeType(char *src)
    {
        ...
        //FIXME:如果出现* + -, 并且它们的下一个字符为空格, 说明是列表
        if (strncmp(ptr, "- ", 2) == 0)
            return make_pair(ul, ptr + 1);

        //如果出现>且下一个字符为空格, 说明是引用
        if (*ptr == '>' && (ptr[1] == ' '))
            return make_pair(quote, ptr + 1);

        //如果出现的是数字, 且下一个字符是., 说明是有序列表
        char *ptr1 = ptr;
